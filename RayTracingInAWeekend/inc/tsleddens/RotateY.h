...
public:
    ...
    {
        Point3 origin( ( m_cosTheta * ray.GetOrigin().x ) - ( m_sinTheta * ray.GetOrigin().z ), ray.GetOrigin().y,
                       ( m_sinTheta * ray.GetOrigin().x ) + ( m_cosTheta * ray.GetOrigin().z ) );

        Vector3 direction( ( m_cosTheta * ray.GetDirection().x ) - ( m_sinTheta * ray.GetDirection().z ),
                           ray.GetDirection().y,
        ...
        if ( isHit )
        {
            ...
            hitResult.SetFaceNormal(
                Point3( ( m_cosTheta * hitResult.GetNormal().x ) + ( m_sinTheta * hitResult.GetNormal().z ),
    ...
    {
        return m_object->HasFlippedNormals();
    }
