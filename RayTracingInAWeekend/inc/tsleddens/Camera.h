...
public:
    ...
    void SetBackgroundColor( const float rgb )
    {
        m_background = Color( rgb );
    }
    ...
    void SetMaxBounces( const UINT max )
    {
        m_maxBounces = max;
    }
    void SetSamplesPerPixel( const UINT value )
    {
    ...
    void DisableRenderNormals()
    {
        m_isRenderNormalsEnabled = false;
    }

    [[nodiscard]] UINT GetSamplesPerPixel() const
    {
        return m_samplesPerPixel;
    }
    [[nodiscard]] UINT GetMaxBounces() const
    {
        return m_maxBounces;
    }

    [[nodiscard]] bool IsRenderNormalsEnabled() const
    {
        return m_isRenderNormalsEnabled;
    }
...
private:
    ...
    [[nodiscard]] Color SampleColor( const Ray& ray, const IRayTraceable& world, UINT currentBounces,
...
{
    return Vector3( RandomFloat( minRange, maxRange ) - 0.5f, RandomFloat( minRange, maxRange ) - 0.5f, 0.f );
}
}  // namespace tsleddens
