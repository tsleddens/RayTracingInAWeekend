...
namespace tsleddens
{
    ...
    public:
        ConstantMedium(const std::shared_ptr<IRayTraceable>& boundary, const float density, std::shared_ptr<ITexture> texture) :
            m_boundary(boundary),
        ...
        [[nodiscard]] bool Intersect(const Ray& ray, HitResult& hitResult, Range<float> range) const override
        {
            ...
            if (hitResult1.GetDistance() < range.GetMin()) hitResult1.SetIntersection(range.GetMin());
            if (hitResult2.GetDistance() > range.GetMax()) hitResult2.SetIntersection(range.GetMax());
        ...
        [[nodiscard]] IMaterial* GetMaterial() const override { return m_phaseFunction.get(); }
        [[nodiscard]] bool HasFlippedNormals() const override { return false; }
        [[nodiscard]] const AABB& BoundingBox() const override { return m_boundary->BoundingBox(); }
    };
}
