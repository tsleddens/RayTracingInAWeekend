...
bool AABB::IsHit(const Ray& ray, Range<float>& minMax) const
{
    ...
    {
        const Range<float>& range = AxisRange(axis);
        ...
        if (tMin < tMax)
        {
            if (tMin > minMax.GetMin()) minMax.SetMin(tMin);
            if (tMax > minMax.GetMax()) minMax.SetMax(tMax);
        }
        else
        {
            if (tMax > minMax.GetMin()) minMax.SetMin(tMax);
            if (tMin > minMax.GetMax()) minMax.SetMax(tMin);
        }
